...
from Strategy.create_lagged_series import create_lagged_series
from Event import SignalEvent, MarketEvent

from datetime import datetime


class ETFForecastStrategy(Strategy):
    """
    ...
    def create_symbol_forecast_model(self):
        ...
        X_train = X[
            X.index > X.index[2]
        ]  # avoid 2 nan values TODO --> filter one is timestamp other datetime index
        ...
        if isinstance(event, MarketEvent):
            ...
            if self.bar_index > 5:
                lags = self.bars.get_latest_bars_values(
                    self.symbol_list[0], "close", N=3
                )

                # series of lags for 2 days prior
                pred_series = pd.Series(
                    {"Lag1": lags[1] * 100.0, "Lag2": lags[2] * 100.0}
                )
